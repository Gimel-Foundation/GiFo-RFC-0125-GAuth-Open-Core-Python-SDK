...
### 2.7 Deployment Pattern → Adapter Type Cross-Reference

| Pattern | Type A (OAuth Engine) | Type B (Foundry/Wallet) | Type C (AI Gov / Web3 / DNA) |
|---------|----------------------|------------------------|------------------------------|
| **Sidecar** | Required (claims provider) | External (not part of sidecar) | External (not part of sidecar) |
| **Gateway** | External (existing OAuth issues tokens) | Optional (user-provided) | Not applicable (requires Full Stack or hybrid) |
| **Full Stack** | Bundled (Hydra) | Bundled or user-provided | Available at M+O/L+O tariff (attested Gimel adapters) |
...
> **Billing boundary:** Self-hosted SDK operations (Open Core PEP enforcement, local PoA validation, rule-based checks) are **never billed** regardless of tariff. The billing adapter only meters operations that invoke Gimel-hosted services (AI governance, attested adapters, managed connectors). A deployment running tariff O with a self-hosted PEP incurs zero platform charges.
...
#### CT-PEP-034: Hybrid cascade — Auth PEP unreachable, rule-based-only fallback (CHK-ESC)
- **Input:** Same as CT-PEP-032 but Auth PEP throws connection error
- **Expected:** `decision: "CONSTRAIN"` (preserves SDK rule-based result), CHK-ESC `fail` severity `warning`, `violation_code: "AUTH_PEP_UNREACHABLE"`
...
#### CT-LIC-002: ToS coexistence activation on Type C registration
- **Input:** Customer with `license_type: "mpl_2_0"` calls `accept-license` for `ai_governance`
- **Expected:** `license_type: "gimel_tos"` (coexists with MPL 2.0), `license_accepted_at` set, `license_version: "2026.1"`
...
| **License/ToS** | — | — | — | — | §1.5.4 (ToS coexistence) |
...
| 0.91 | 2026-04-13 | Auth Team | **Public Preview.** Licensing reframed to coexistence model (MPL 2.0 always applies to SDK code; Gimel Technologies ToS applies additionally for proprietary services). Downgrade protection clause added. Three-layer legal table. Added hybrid tariff codes: M+O (`gimel_mo_hybrid_service`, effective level M) and L+O (`gimel_lo_hybrid_product`, effective level L) with `openCoreActive` flag and `tariffEffectiveLevel()` resolver. Added `PoaMapSummary` type with `permissions`, `allowedActions`, `allowedDecisions` fields. Added `checkTariffGate()` runtime gating function with full deployment policy matrix. PEP hybrid cascade escalation: CONSTRAIN decisions optionally forwarded to Auth PEP via `AuthPEPClient` protocol; Auth PEP unreachable → rule-based-only fallback (CONSTRAIN preserved). 6 new conformance test vectors (CT-PEP-032 through CT-PEP-037). All packages bumped to 0.91.0. Self-hosted SDK operations billing boundary note added (§6.2). |