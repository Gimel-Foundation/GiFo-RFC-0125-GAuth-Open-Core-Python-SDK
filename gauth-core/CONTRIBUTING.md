...
- **Community PRs (Stream A):** External developers submit pull requests on GitHub. These are reviewed and approved by the Gimel Foundation Board of Trustees for code quality, spec alignment, license compliance, and security.
- **Architecture team pushes (Stream B):** The Gimel architecture team pushes changes from the development sandbox. These go through internal review.

Both streams merge to `main`. Neither stream automatically triggers a version bump or release. Only the architecture team decides when to cut a new version — version numbers are architectural decisions, not merge counters. See the SDK Implementation Guide §16 for the full versioning and release policy.
